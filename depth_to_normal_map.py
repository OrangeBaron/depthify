...
class DepthToNormalMap:
    ...
    def __init__(self, depth_map_path: str, max_depth: int = 255) -> None:
        ...
        """
        self.depth_map = cv2.imread(depth_map_path, cv2.IMREAD_UNCHANGED).astype(np.float32)
    ...
    def convert(self, output_path: str) -> None:
        ...
        # Normalize the depth values to be in the range [0, 1]
        normalized_depth = self.depth_map / self.max_depth

        # Calculate the partial derivatives of depth with respect to x and y
        dx = cv2.Sobel(normalized_depth, cv2.CV_32F, 1, 0)
        dy = cv2.Sobel(normalized_depth, cv2.CV_32F, 0, 1)
